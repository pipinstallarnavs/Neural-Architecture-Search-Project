"""
ResNet Ensemble Surrogate Model
"""
...
class ResNetRegressor(nn.Module):
    """ResNet-style regressor with residual blocks."""
    
    def __init__(self, input_dim: int, hidden_dim=128, num_blocks=4):
        super().__init__()
        self.fc_in = nn.Linear(input_dim, hidden_dim)
        self.blocks = nn.ModuleList([ResBlock(hidden_dim, hidden_dim) for _ in range(num_blocks)])
...
class ResNetEnsemble(SurrogateModel):
    """Ensemble of ResNet regressors."""
    ...
    def __init__(self, input_dim: int, hidden_dim=128, num_blocks=4, 
        ...
        for _ in range(ensemble_size):
            model = ResNetRegressor(input_dim, hidden_dim, num_blocks).to(device)
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, epochs=200, batch_size=64, 
        ...
        for ep in range(epochs):
            ...
            if patience and max(waits) > patience:
                if verbose:
                    print(f"[ResNet Ensemble] Early stop at epoch {ep}")
                break
